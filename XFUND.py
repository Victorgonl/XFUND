    ...
    def _generate_examples(self, filepaths):
        for filepath in filepaths:
            ...
            for doc in data["documents"]:
                ...
                chunk_size = CHUNK_SIZE - 2  # for +2 special tokens on start end end
                for chunk_id, index in enumerate(
                    ...
                    item["bbox"] = [CLS_TOKEN_BOX] + item["bbox"] + [SEP_TOKEN_BOX]
                    item["labels"] = [O_LABEL] + item["labels"] + [O_LABEL]
                    item["attention_mask"] = [1 for _ in range(len(item["input_ids"]))]
                    item = self.tokenizer.pad(
                        item,
